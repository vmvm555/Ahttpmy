...
public abstract class AsyncHttpResponseHandler implements
	...
	public void setUseSynchronousMode(boolean sync) {
		...
		// 在设置异步模式之前必须设置好looper,否则强制设为同步模式
		if (!sync && looper == null) {
			sync = true;
			Log.w(LOG_TAG,
	...
	/**
	 * 将状态码,响应头,响应体封装成msg,并和handle关联上发送出去
	 * 调用这个方法,视为发送成功
	 */
	@Override
	...
	protected void handleMessage(Message message) {
		...
		try {
			...
			case FAILURE_MESSAGE:
				...
				break;
			case START_MESSAGE:
				// 最终调用这个方法,在请求发出之前,可能要处理UI(能执行到这里要么handle为null,要么为同步模式,线程运行在UI线程中,可以直接处理UI)
				onStart();
				break;
			case FINISH_MESSAGE:
				onFinish();
	...
	protected void sendMessage(Message msg) {
		// 当同步或者handle为null的时候,由本省来处理这个msg
	...
	/**
	 * 如果响应码>300的时候,视为失败,发送失败信息
	 * 否则将响应码,响应头,响应体视为成功发送出去
	 */
	@Override
