...
public class AsyncHttpClient {
	 ...
	 * 
	 * 将请求,Context,httpclient, responseHandler再度封装一层
	 */
	...
	protected RequestHandle sendRequest(DefaultHttpClient client,
		...
		// 在响应handle中设置请求头,但是并没用上
		responseHandler.setRequestHeaders(uriRequest.getAllHeaders());
		// 在响应handle中设置请求URI,但是并没用上
		...
		//将请求封装进RequestHandle,最后将requestHandle扔进集合,每个请求对应一个requesthandle
		...
		if (context != null) {
			...
			// 将请求集合添加进map
			// 因为本类是AsynchttpClient,只有一个实例,所以要用集合存储所有REquestHandle
			...
			//  每次添加请求进线程池后,要检查一遍请求中是否有执行玩的,有的话,清除请求
